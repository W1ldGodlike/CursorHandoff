...
[![Version](https://img.shields.io/badge/version-1.6.1-blue)](https://github.com/W1ldGodlike/CursorHandoff/releases)
...
| **Run / Skip / Confirm search / Delete / Generate image approvals** | Cloud agents сами жмут терминал; у RC другая модель | ✅ Те же карточки, что в IDE (shell, веб-поиск, удаление файла, генерация картинок) |
...
| Область | Что даёт |
|---------|----------|
| **Веб-клиент** | Живая лента, карточки Run/Skip (shell, Confirm search, Delete file, **Generate image**; опциональный звук approve), **превью сгенерированных картинок**, виджеты плана (View Plan / Build), код и диффы, вложения (картинки — paste, остальное — путь к файлу), очередь и `$` |
| **Telegram** | Топик форума на вкладку, слэш-команды, входящие файлы (фото, видео, голос, документы), исходящий [file relay](docs/telegram.md) из `.cursor-handoff/outbox/`, **авто `sendPhoto` для сгенерированных агентом картинок** |
...
|-------|------|--------|----------------|
| **Standard** | `cursor-handoff-1.6.1.vsix` | ~2 MB | Меньше вес; Wake и cloudflared — кнопка **Скачать и установить** в Handoff settings (GitHub / CDN Cloudflare) |
| **Complete** | `cursor-handoff-1.6.1-complete.vsix` | ~43 MB | **Дополнения в пакете** — `CursorWake.exe` + `cloudflared.exe` (Windows) уже в VSIX; установка из Handoff settings **без отдельной докачки** |
...
```bash
cursor --install-extension cursor-handoff-1.6.1.vsix
# или Complete:
cursor --install-extension cursor-handoff-1.6.1-complete.vsix
```
...
**Пример VSIX (Windows, Cursor):** `%USERPROFILE%\.cursor\extensions\cursor-handoff.cursor-handoff-1.6.1\data\`  
VS Code: `~/.vscode/extensions/cursor-handoff.cursor-handoff-1.6.1/data/`
