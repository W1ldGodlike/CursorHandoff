...
export class MessageTracker {
  ...
  track(
    ...
    this.pruneThread(threadId);
    this.scheduleSave();
  }

  /** Drop old thread entries over cap (Map preserves insertion order). */
  private pruneThread(threadId: number): void {
    let count = 0;
    for (const msg of this.messages.values()) {
      if (msg.threadId === threadId) count++;
    }
    if (count <= MAX_PER_THREAD) return;
    let toDrop = count - MAX_PER_THREAD;
